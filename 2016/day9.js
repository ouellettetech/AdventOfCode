...
function decompress(compressedString){
	...
	var numChars;
	var repeat;
	var nextIndex=compressedString.indexOf("(");

	while(nextIndex!==-1){
		uncompressed=uncompressed+compressedString.slice(0,nextIndex);
		compressedString=compressedString.slice(nextIndex);
		markerIndex = compressedString.indexOf(")");
		marker = compressedString.slice(1,markerIndex);
		//console.log(marker);
		marker = marker.split("x");
		numChars=parseInt(marker[0]);
		repeat=parseInt(marker[1]);
		var repeatString = compressedString.slice(markerIndex+1,markerIndex+1+numChars);
		for(var i=0;i<repeat;i++){
			uncompressed+=repeatString;
		}
		compressedString = compressedString.slice(markerIndex+1+numChars);
		nextIndex=compressedString.indexOf("(");
	}
	uncompressed+=compressedString;
	return uncompressed;
}

function decompressLength(compressedString){
	var totalLength = 0;
	var markerIndex;
	var marker;
	var numChars;
	...
	while(nextIndex!==-1){
		...
		var repeatString = compressedString.slice(markerIndex+1,markerIndex+1+numChars);
		for(var i=0;i<repeat;i++){
			totalLength+=decompressLength(repeatString);
		}
		compressedString = compressedString.slice(markerIndex+1+numChars);
		nextIndex=compressedString.indexOf("(");
	}
	totalLength+= compressedString.length;
	return totalLength;
}
...
function testData(){
	...
	var test2="A(1x5)BC";
	var test3="(3x3)XYZ";
	var test4="A(2x2)BCD(2x2)EFG";
	var test5="(6x1)(1x3)A";
	var test6="X(8x2)(3x3)ABCY";
	var test7="(27x12)(20x12)(13x14)(7x10)(1x12)A";
	var test8="(25x3)(3x3)ABC(2x3)XY(5x2)PQRSTX(18x9)(3x2)TWO(5x7)SEVEN";
	console.log(decompress(test1));
	console.log(decompress(test1).length);
	console.log(decompress(test2));
	console.log(decompress(test2).length);
	console.log(decompress(test3));
	console.log(decompress(test3).length);
	console.log(decompress(test4));
	console.log(decompress(test4).length);
	console.log(decompress(test5));
	console.log(decompress(test5).length);
	console.log(decompress(test6));
	console.log(decompress(test6).length);

	console.log("Part 2");
	console.log(decompressLength(test1));
	console.log(decompressLength(test2));
	console.log(decompressLength(test7));
	console.log(decompressLength(test8));
}


function day9a(compressedString) {
	console.log(decompress(compressedString).length);
}

function day9b(compressedString) {
	console.log(decompressLength(compressedString));
}

testData();
console.log("Day 9 Part A");
day9a(compressed);
console.log("Day 9 Part B");
day9b(compressed);
