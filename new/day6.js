...
function testData2(){
	...
	var myGrid = new Grid();
	myGrid.setField(0,0,999,999,0);
	console.log(myGrid.countValue());
	runOneInstructionB(instruction1,myGrid);
	console.log(myGrid.countValue());
	runOneInstructionB(instruction2,myGrid);
	console.log(myGrid.countValue());
	runOneInstructionB(instruction3,myGrid);
	console.log(myGrid.countValue());
}


function runOneInstructionA(inputString,lights){
	var inputArray=inputString.split(" ");
	if(inputArray[0]==="toggle"){
		var point1 = inputArray[1].split(",");
		var point2 = inputArray[3].split(",");
		lights.toggle(parseInt(point1[0]),parseInt(point1[1]),parseInt(point2[0]),parseInt(point2[1]));
	} else {
		if(inputArray[1]==="on"){
			var point1 = inputArray[2].split(",");
			var point2 = inputArray[4].split(",");
			lights.toggleOn(parseInt(point1[0]),parseInt(point1[1]),parseInt(point2[0]),parseInt(point2[1]));
		} else {
			if(inputArray[1]==="off"){
				var point1 = inputArray[2].split(",");
				var point2 = inputArray[4].split(",");
				lights.toggleOff(parseInt(point1[0]),parseInt(point1[1]),parseInt(point2[0]),parseInt(point2[1]));
			} else {
				console.log("Error Can't parse string: "+inputString);
			}		
		}
	}
}

function runOneInstructionB(inputString,lights){
	var inputArray=inputString.split(" ");
	if(inputArray[0]==="toggle"){
		var point1 = inputArray[1].split(",");
		var point2 = inputArray[3].split(",");
		lights.incrementValues(parseInt(point1[0]),parseInt(point1[1]),parseInt(point2[0]),parseInt(point2[1]),2);
	} else {
		if(inputArray[1]==="on"){
			var point1 = inputArray[2].split(",");
			var point2 = inputArray[4].split(",");
			lights.incrementValues(parseInt(point1[0]),parseInt(point1[1]),parseInt(point2[0]),parseInt(point2[1]),1);
		} else {
			if(inputArray[1]==="off"){
				var point1 = inputArray[2].split(",");
				var point2 = inputArray[4].split(",");
...
function day6a(instructions){
	var runInst = instructions.split("\n");
	var lights = new Grid();
	lights.toggleOff(0,0,999,999);
	for(var i=0;i<runInst.length;i++){
		runOneInstructionA(runInst[i],lights);
	}
	console.log(lights.countTrue());
}

function day6b(instructions){
	var runInst = instructions.split("\n");
	var lights = new Grid();
	lights.setField(0,0,999,999,0);
	for(var i=0;i<runInst.length;i++){
		runOneInstructionB(runInst[i],lights);
	}
	console.log(lights.countValue());
}

function Grid(){
	this.grid=[];

	this.toggleOn = function(x1,y1,x2,y2){
		for(var i=x1;i <= x2 ;i++){
			for(var j=y1;j<=y2;j++) {
				this.set(i,j,true);
			}
	...
	this.toggleOff = function(x1,y1,x2,y2){
		for(var i=x1;i <= x2 ;i++){
			for(var j=y1;j<=y2;j++) {
				this.set(i,j,false);
			}
...
//				console.log(curRow);
	...
	this.get = function(x,y){
		if(!this.grid[x]){
			return 0; //first row doesn't exist.
		}
